...
def shlex_quote(args: Sequence[str]):
...
async def Await(
    self: "aexpect.Spawn", timeout: int | float | None = None, interval=_INTERVAL
):
    ...
    if self.is_alive():
        if not no_kill:
            ...
            set_status(
                self, 128 + 15
            )  # SIGTERM, fix aexpect always return 0 even if killed
        else:
        ...
        if self.get_status() == 128 + 9:  # SIGKILL
            Log.error(
                "The process is forced to be terminated by the system (-9), which may be caused by insufficient memory (OOM). Please check the system memory or issue to developer. 进程被系统强制终止（-9），可能是内存不足（OOM）导致，请检查系统内存或联系开发者。"
            )
    return self
...
class Spawn(aexpect.Spawn):
    @copy_args(Await)
    async def Await(self, *args, **kwargs):
        return await Await(self, *args, **kwargs)
        ...
        if kwargs.get("output_func", None) is None:
            ...
            def _Log_info(msg: str, *args, **kwargs):
                (
                    ...
                    if _RE_CTRL_ASCII.sub("", msg).strip()
                    else None
                )
...
def run_fg(cmds: Sequence[str], *args, **kwargs) -> tuple[int | None, str | None]:
    return _aexpect("fg", aexpect.run_fg)(cmds, *args, **kwargs)


@copy_args(aexpect.run_bg)
def run_bg(cmds: str | Sequence[str], *args, **kwargs) -> Expect:
    return _aexpect("bg", aexpect.run_bg)(cmds, *args, **kwargs)


@copy_args(aexpect.run_tail)
def run_tail(cmds: str | Sequence[str], *args, **kwargs) -> Tail:
    return _aexpect("", aexpect.run_tail)(cmds, *args, **kwargs)


async def unzip(
    zip_path: str | Path,
    From="",
    to: str = CONFIG["search_dir"],
    pwd="",
...
async def Python(
    arg0: str | Path = "", *args: str, run: TYPE_RUNS | Path, env="default"
):
    ...
    py = (
        ...
        else [str(res_path(module="run", file=f"{run}.py")), *_arg0]
    )
