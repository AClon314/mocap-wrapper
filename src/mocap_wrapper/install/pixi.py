...
if __name__ != "__main__":
    raise ImportError(
        "This installation script must be run directly, not imported as a module."
    )
import os
import re
import sys
...
MIRROR_CN_PY = (
    ...
    else "https://raw.githubusercontent.com/AClon314/mirror-cn/main/src/mirror_cn/mirror_cn.py"
)
_HOME = os.path.expanduser("~")
PIXI_BIN = os.getenv("PIXI_BIN", None) or os.path.join(_HOME, ".pixi", "bin")
VENV = os.path.join(_HOME, ".venv")
os.environ["PATH"] = os.pathsep.join(
    [os.getenv("PATH", ""), PIXI_BIN, os.path.join(VENV, "bin")]
)
        ...
        if elapsed > TIMEOUT:  # 连接稳定后开始判断
            if last_speed < SLOW_SPEED:
                ...
                # 持续低速超过阈值时间则触发重试
                    ...
                    start_slow_time = None
                    raise Exception(
                        f"🐌 Too slow, speed={last_speed:.2f}MB/s < {SLOW_SPEED}MB/s for {TIMEOUT}s."
                    )
            else:
                start_slow_time = None
...
def download(
    ...
    if_exist: Literal["override", "skip"] = "skip",
    log=True,
):
...
def i_mocap():
    ...
    git = (
        "https://gitee.com/AClon314/mocap-wrapper"
        if IS_MIRROR
        else "https://github.com/AClon314/mocap-wrapper"
    )
    install = (
        ["-e", ".[dev]"]
        if os.getcwd().endswith(_pkg_.replace("_", "-"))
        else [f"git+{git}"]
    )
...
"""
    ...
    if not shells:
        Log.warning(
            f"⚠️⚠️⚠️ venv/PATH update at {profile}, add to your .*shrc: {_activate} ⚠️⚠️⚠️"
        )
    return _activate
...
def is_unattended():
    parser = argparse.ArgumentParser(
        description=f"Pre-install script for {_pkg_}. {_pkg_}的预安装脚本。"
    )
    parser.add_argument(
        "-y",
        "--yes",
        action="store_true",
        help="Unattended, only pre-install. 无人值守，仅预安装。",
    )
    args = parser.parse_args()
    if not args.yes:
        confirm = (
            input("Install pixi & uv as python env manager, and mocap-wrapper? (y/N): ")
            .strip()
            .lower()
        )
        if confirm != "y":
