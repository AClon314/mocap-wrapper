...
TYPE_HUGFACE = (
    TYPE_RUNS | Literal["smplx", "hamer", "hawor"]
)  # TODO: remove hamer/hawor if supported in run
...
def get_repo_id_type(repo: TYPE_HUGFACE):
    """```python
    repo_id, repo_type = get_repo_id_type(repo)
    ```"""
...
async def i_hugging_face(*run: TYPE_HUGFACE, concurrent=2):
    ...
    Log.info(
        f"📦 Download {run} models (📝 By downloading, you agree to the corresponding licences)"
    )
    ...
    for repo in repos:
        ...
        remote_paths = await asyncio.to_thread(
            Env.HF.list_repo_files, repo_id=repo_id, repo_type=repo_type
        )  # request 1 by 1
        remote_paths = [
            f for f in remote_paths if not (f.startswith(".") or f.startswith("README"))
        ]  # 排除隐藏文件
        ...
        for str_path in remote_paths:
            ...
            if repo_in_run := repo in run:
                ...
                _locals += [
                    Path(
                        CONFIG.get(repo, ""), REPO_TO_LOCAL_PREFIX.get(repo, ""), Rpath
                    )
                ]
    ...
    async def dl(self=Env.HF, *args, dsts: list[Path], **kwargs):
        ...
        _dir = (
            _dst.resolve().parent if Path(filename).parent == _dst.parent.name else _dst
        )  # TODO: hf_hub_download replicate file structure! like dpvo/dpvo/dpvo.ckpt
