...
async def i_gvhmr(Dir: Union[str, Path] = CONFIG[_STEM]):
...
def link_config(Dir: Union[Path, str] = CONFIG[_STEM], file="gvhmr.yaml"):
...
async def i_dl_models():
    ...
    if not p:
        Log.error(
            f"Please download GVHMR models at https://huggingface.co or https://drive.google.com/drive/folders/1eebJ13FUEXrKBawHpJroW0sNSxLjh9xD?usp=drive_link"
        )
    return p


async def i_dpvo(Dir: Union[Path, str] = CONFIG["search_dir"]):
    Log.info("📦 Install DPVO")
    f = File(
        "https://gitlab.com/libeigen/eigen/-/archive/3.4.0/eigen-3.4.0.zip",
        path=Path(Dir, "eigen-3.4.0.zip"),
