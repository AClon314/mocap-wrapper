...
try:
    ...
    def replace_github_with_mirror(file: str) -> Generator[tuple[str, str], Any, None]:
        yield ((file, "github.com"))
...
async def install(runs: Sequence[TYPE_RUNS], **kwargs):
    ...
    if "wilor" in runs:
        ...
        [
            ...
            asyncio.create_task(wait_all_dl()),
        ],
...
async def i_python_env(
    ...
    timeout = (
        4 if Env.is_mirror and use_mirror else TIMEOUT_QUATER
    )  # fail quickly if CN use github.com
    Log.debug(f"{locals()=}")
    iter_github = [(_toml, "github.com")]
    iters = (
        itertools.chain(iter_github, replace_github_with_mirror(file=str(_toml)))
        if use_mirror
        else iter_github
    )
...
async def Git(
    ...
    """with mirror_cn"""
    Log.debug(f"Git {cmd=}")
    err_msg = f"Git {' '.join(cmd)}, failed at {os.getcwd()}, check ~/.gitconfig for mirror setting"
    if Env.is_mirror:
...
async def gather(coros: Sequence[Coroutine], success_msg="", Raise=False):
    ...
    if exceptions:
        if Raise:
            raise (
                ExceptionGroup("", exceptions) if len(exceptions) > 1 else exceptions[0]
            )
        else:
