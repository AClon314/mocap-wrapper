...
def i_dpvo(setup_py=Path(CONFIG[_STEM], 'third-party', 'DROID-SLAM', 'setup.py')): return Python(setup_py, 'install', run=_STEM)    # '''install DROID-SLAM with CUDA compiling job'''
...
def i_bmc(Dir=Path(CONFIG[_STEM], 'Hand-BMC-pytorch')):
    ...
    if_missing_then_calc = {
        Path(CONFIG[_STEM], '_DATA', 'BMC', 'bone_len_max.npy'): Python(Dir.joinpath('calculate_bmc.py'), run='dynhamr', env='bmc'),
        Path(CONFIG[_STEM], '_DATA', 'BMC', 'CONVEX_HULLS.npy'): Python(Dir.joinpath('calculate_convex_hull.py'), run='dynhamr', env='bmc'),
    }
    queue = []
    _gather = []
    if not Path(Dir).exists():
        _gather.append(Git(['clone', 'https://github.com/ZhengdiYu/Dyn-HaMR', str(Dir)]))
    _gather.append(i_python_env(Dir=Dir, pixi_toml=f'{_STEM}.toml', env='bmc', use_mirror=False))
    ...
    if is_linux:
        os.environ['QT_QPA_PLATFORM'] = 'xcb'   # fix: This application failed to start because it could not find or load the Qt platform plugin "wayland" in "". Available platform plugins are: eglfs, minimal, minimalegl, offscreen, vnc, xcb.
    return run_1by1(queue)
