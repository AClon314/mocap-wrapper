...
def version():
    return f"{PACKAGE} {VERSION} 👻\tconfig: {CONFIG.path}\tcode: https://github.com/AClon314/mocap-wrapper"
...
async def run(
    runs: Sequence[TYPE_RUNS],
    input: str,
    outdir: str,
    Range="",
    args: Sequence[str] = [],
):
    video = await ffmpeg_or_link(input, outdir, Range=Range) if input else ""
    for m in runs:
...
def argParse():
    print(_VERSION_)
    arg = ArgParser(
        description=f"sincerelly thanks to gvhmr/wilor/wilor-mini devs and others that help each other♥️ , please consider donate♥️ if helps you a lot :)"
    )
    arg.add_argument("-v", "--version", action="store_true")
    arg.add_argument(
        "-I", "--install", action="store_true", help="force to re-install runs"
    )
    arg.add_argument(
        "-b",
        "--by",
        ...
        default=False,
        metavar=",".join(RUNS),
        help=f'install with/run by, default all installed, eg: `--by={",".join(DEFAULT)}`',
    )
    arg.add_argument(
        "-@",
        "--at",
        metavar=CONFIG["search_dir"],
        help='search_dir of git repos, eg: `--at=".."` if GVHMR is current work dir',
    )
    ...
    arg.add_argument(
        "-r",
        "--range",
        metavar="[a,b]or[a,duration]",
        default="",
        help="video time range, eg: `--range=0:0:1,0:2` is 1s~2s, `--range=10` is 0s~10s",
    )
...
async def mocap(
    ...
    outdir=OUTPUT_DIR,
    Range="",
    at=CONFIG["search_dir"],
    by: Sequence[TYPE_RUNS] = RUNS,
    args: list[str] = [],
):
    if at:
    ...
    for i in by:
        if (
            i
            and getattr(CONFIG, i, True)
            and os.path.exists(os.path.join(CONFIG[i], "pixi.toml"))
        ):
            _by.remove(i)
